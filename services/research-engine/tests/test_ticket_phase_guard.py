...
- Proof: `PYTHONPATH=src ./.venv/bin/python -m pytest -q`
"""
...
- Partial run only.
- Proof: `pytest -q tests/test_stack_phase1_adapters.py`
"""
...
def test_phase2_blocked_with_unverified_phase1_result(tmp_path: Path) -> None:
    (tmp_path / "stack_phase1_akshare_qlib_adapter__RESULT__20260305T203503Z.md").write_text(
        FAILING_RESULT,
        encoding="utf-8",
    )
    allowed, reason = phase_gate_status("stack_phase2_rd_agent_hypothesis_lane", tmp_path)
    assert allowed is False
    assert reason == "blocked_waiting_for_stack_phase1_akshare_qlib_adapter_verified_result"
